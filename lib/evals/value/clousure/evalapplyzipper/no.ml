...
let rec no (t, zipp) =
  match t with
    ...
    | Clou(CVar x, (y, n)::env) ->
      if x = y then no (n, zipp) else no @@ (Clou(CVar x, env), zipp)
    | CAbs(x, b) ->
      let (b', _) = no (b, CAbsC(x, zipp)) in (CAbs(x, b'), zipp)
    | Clou(CAbs(x, b), env) ->
      let var = x ^ "*" ^ string_of_int (cl_fresh ()) in
      let (b', _) = no @@ (Clou(b, (x, CVar var)::env), CClouC(zipp, env)) in
        (CAbs(var, b'), zipp)
    | CApp(m, n) ->
    ...
    | Clou(Clou(t, env1), env2) -> no @@ (Clou(t, env1 @ env2), zipp)
