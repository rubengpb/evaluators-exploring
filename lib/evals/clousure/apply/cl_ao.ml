...
let rec eval_cl_ao ct =
  match ct with
    ...
    match t, ctx with
      ...
      | CVar x as var , (y, v)::c ->
        if x = y then v
        else eval_cl_ao (Clou (var, c))
      | CAbs (x, b), c ->
        Clou (CAbs (x, eval_cl_ao (Clou (b, clean_ctxt x c))), [])
      | CApp (m, n), c ->
        apply (eval_cl_ao (Clou(m,c))) (eval_cl_ao (Clou (n, c)))
...
and apply m n =
  match m with
    | Clou (CAbs(x, b), c) -> eval_cl_ao (Clou(b, (x, n)::c))
    | _ -> Clou(CApp(m, n), [])
