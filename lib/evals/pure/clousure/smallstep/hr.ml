open Core.Syntax
open Core.Utils
open Cl_utils
open Core.Forms
open Bn
open Printer

let rec step_hr = function
  | Clou(CVar x, []) -> CVar x
  | Clou(CVar x, (y, n)::env) ->
    if x = y then n
    else step_hr (Clou(CVar x, env))
  | CAbs(x, b) when not (is_chnf b) ->
    let b' = step_hr b in
    CAbs(x, b')
  | Clou(CAbs(x, b), env) ->
    let var = x ^ "*" ^ string_of_int (cl_fresh ()) in
    let b' = step_hr @@ Clou(b, (x, CVar var)::env) in
      CAbs(var, b')
  | CApp(m, n) when not (is_cwhnf m) ->
    let m' = step_bn m in
    CApp(m', n)
  | CApp (CAbs(x, b), n) ->
    Clou(b, [(x, n)])
  | CApp (Clou(CAbs(x, b), env), n) ->
    Clou(b, (x, n)::env)
  | CApp(m, n) when not (is_chnf m) ->
    let m' = step_hr m in
    CApp(m', n)
  | Clou(CApp(m, n), env) ->
    step_hr (CApp(Clou(m, env), Clou(n, env)))
  | Clou(Clou(t, env1), env2) ->
    step_hr (Clou(t, env1 @ env2))
  | _ -> failwith "not redex!"

let rec string_of_term_ss_hr = string_of_cterm

let rec hr t =
  if is_chnf t then t
  else (
    print_endline @@ string_of_term_ss_hr t;
    t |> step_hr |> hr
  )
