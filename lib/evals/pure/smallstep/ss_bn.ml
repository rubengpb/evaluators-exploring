...
let rec string_of_term_ss_bn = function
  | Var x -> x
  | Abs (x, t) ->
      "\\" ^ x ^ "." ^ string_of_term_ss_bn t 
  | App (Var x1, Var x2) -> x1 ^ " " ^ x2
  | App (Var x, t) -> x ^ " (" ^ string_of_term_ss_bn t  ^ ")"
  | App (App(t1, t2), Var x) -> string_of_term_ss_bn (App (t1, t2))  ^ " " ^ x
  | App (App(t1, t2), Abs(x,t)) -> string_of_term_ss_bn (App (t1, t2))  ^ " (" ^ string_of_term_ss_bn (Abs(x,t))  ^ ")"
  | App (App(t1, t2), t) -> string_of_term_ss_bn (App (t1, t2)) ^ " (" ^ string_of_term_ss_bn t ^ ")"
