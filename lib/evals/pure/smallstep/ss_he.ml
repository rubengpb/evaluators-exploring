...
let rec step_he = function
  | Abs (x, body) -> Abs(x, step_he body)
  | App (Abs (x, body), arg) -> subst arg x body
  | App (Var x, arg) -> App (Var x, step_he arg)
  | App (t1, t2) -> App(step_he t1, t2)
  | t -> t (* Variables *)
...
let rec string_of_term_ss_he = function
  ...
  | App (App(t1, t2), Abs(x,t)) -> string_of_term_ss_he (App (t1, t2))  ^ " (" ^ string_of_term_ss_he (Abs(x,t))  ^ ")"
  | App (App(t1, t2), t) -> string_of_term_ss_he (App (t1, t2)) ^ " (" ^ string_of_term_ss_he t ^ ")"
