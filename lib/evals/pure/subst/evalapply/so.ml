...
let rec so_zipp (t, z_ctxt) =
  match t with
    | Var _ as v -> (v, z_ctxt)
    | Abs (x, b) ->
      let (b', _) = so_zipp (b, AbsC(x, z_ctxt)) in
        (Abs(x, b'), z_ctxt)
    | App (m, n) ->
      let (m', _) = ho_zipp (m , AppL(z_ctxt, n)) in
      let (n', _) = so_zipp (n, AppR(m', z_ctxt)) in
      match m' with
        | Abs(x, b) ->
          let redex_str = string_of_redex (App(m', n')) in
          let full_str = plug_str redex_str z_ctxt in
          print_endline full_str;
          so_zipp (subst n' x b, z_ctxt)
        | _ -> let (m'', _) = so_zipp (m' , AppL(z_ctxt, n)) in
            (App(m'', n'), z_ctxt)
