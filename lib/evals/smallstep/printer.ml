...
let rec string_of_term_ss t =
  match t with
  | Var x -> x
  | Abs (x, t) ->
      "\\" ^ x ^ "." ^ string_of_term_ss t 
  | App (Var x1, Var x2) -> x1 ^ " " ^ x2
  | App (Var x, t) -> x ^ " (" ^ string_of_term_ss t  ^ ")"
  | App (App(t1, t2), Var x) -> string_of_term_ss (App (t1, t2))  ^ " " ^ x
  | App (App(t1, t2), Abs(x,t)) -> string_of_term_ss (App (t1, t2))  ^ " (" ^ string_of_term_ss (Abs(x,t))  ^ ")"
  | App (App(t1, t2), t) -> string_of_term_ss (App (t1, t2)) ^ " (" ^ string_of_term_ss t ^ ")"
