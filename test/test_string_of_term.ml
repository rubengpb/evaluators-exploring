...
let test_simple_var () =
  let t = Var "x" in
  check string "simple variable" "x" (string_of_pterm t)

let test_simple_abs () =
  let t = Abs("x", Var "y") in
  check string "simple abstraction" "\\x.y" (string_of_pterm t)

let test_simple_application () =
  let t = App (Var "f", Var "x") in
  check string "application" "f x" (string_of_pterm t)

let test_application_var_abs () =
  let t = App (Var "f", Abs("x", Var "x")) in
  check string "application" "f (\\x.x)" (string_of_pterm t)

let test_application_app_var () =
  let t = App(App(Var "s", Var "z"), Var "s") in
  check string "application" "s z s" (string_of_pterm t)

let test_application_var_abs () =
  let t = App(Var "s", Abs("s", Var "z")) in
  check string "application" "s (\\s.z)" (string_of_pterm t)
...
let test_application_abs_app () =
  ...
  check string "application" "(\\s.x) (s z)" (string_of_pterm t)

let test_application_abs_var () =
  let t = App(Abs("s", Var "x"), Var "s") in
  check string "application" "(\\s.x) s" (string_of_pterm t)

let test_application_app_app () =
  let t = App(App(Var "s", Var "x"), App(Var "s", Var "x")) in
  check string "application" "s x (s x)" (string_of_pterm t)
