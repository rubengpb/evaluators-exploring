...
let basic_neu () =
  ...
  check string "EvalApply basic neu" result (string_of_term r_ea);
  (* check string "EvalReadBack basic neu" result (string_of_term r_rb); *)
  check string "EvalSmallStep basic neu" result (string_of_term r_ss);
  check string "Eval Zipper basic neu" result (string_of_term r_zi);
  check string "Eval Gen basic neu" result (string_of_term r_gen)
...
let add_100_100 () =
  ...
  check bool "Addition" true (is_whnf r_ea);
  (* check bool "Addition" true (alpha_equiv result r_rb); *)
  check bool "Addition" true (is_whnf r_ss);
  check bool "Addition" true (is_whnf r_zi);
  check bool "Addition" true (is_whnf r_gen)
...
let prod_10_12 () =
  ...
  check bool "Prod" true (is_whnf r_ea);
  (* check bool "Prod" true (alpha_equiv result r_rb); *)
  check bool "Prod" true (is_whnf r_ss);
  check bool "Prod" true (is_whnf r_zi);
  check bool "Prod" true (is_whnf r_gen)
...
let pow_3_4 () =
  ...
  check bool "Pow" true (is_whnf r_ea);
  (* check bool "Pow" true (is_whnf r_rb); *)
  check bool "Pow" true (is_whnf r_ss);
  check bool "Pow" true (is_whnf r_zi);
  check bool "Pow" true (is_whnf r_gen)
...
let () =
  run "eval" [
    ("beta", [
      ...
      test_case "Omega in ignoted argamuent" `Quick not_diver;
